% This code is a modified version of MP_HS_DHSI_for_DME.m from Tou, et. al, 2020 
...
%        xlswrite(dataFile,A,sheet,xlRange)
    ...
    %                 startIndex = 1; endIndex = 100;
            %norm_samples = lhsdesign(40,2,'smooth','off','criterion','maximin','iterations',1000);

                    model='Model-1';parameter='H2=0.005,PD=0.1,MAF=0.05';
                    filepath='.\DME_DATA\DMEmodel1_11_EDM-1\DMEmodel1_11_EDM-1_';


             max_iter =5000; % min([Dim * 100,100000]);
           maxIterForLocalSearch = 18;
           NCM=0;
...
%                  end
                ...
                data = dlmread(strcat(filepath,'008','.txt'),'\t',1,0);


                   %% Search k-snp loci using Harmony search algorithm
                 [Candidate,canSize,Nc,runtime,flag] =NEW_HS(data,epi_dim,HMS,max_iter,maxIterForLocalSearch,CandidateSize,CX );
             ...
             % end harmony search  ****************************************
               ...
               %% 1st stage power
                if flag > 0
                     ...
                     NCM=Nc+NCM;
                     fprintf( '  search time(%f)|FEs=%d,   %d/%d    successfully(^V^)    ' , runtime,Nc,power1 ,1);
                else
                    fprintf(' search time(%f)|FEs=%d   %d/%d   search failed!!!!!! ! ',runtime,Nc,power1, 1);
                end
...
%        JE_Power = JE_power/100;
    ...
    %    ��������(True Positive Rate��TPR)��������(Sensitivity)���ٻ���(Recall)��
